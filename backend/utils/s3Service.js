...
const s3Client = new S3Client({
  region: process.env.AWS_REGION || "YOUR_AWS_REGION", // Replace YOUR_AWS_REGION or set AWS_REGION env var
});

const S3_BUCKET_NAME = process.env.S3_BUCKET_NAME || "YOUR_S3_BUCKET_NAME"; // Set S3_BUCKET_NAME env var
...
const uploadToS3 = async (file, folder = "") => {
  ...
  try {
    ...
    const fileUrl = `https://${S3_BUCKET_NAME}.s3.${s3Client.config.region}.amazonaws.com/${fileName}`;
    return {
